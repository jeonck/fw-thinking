...
## 1. 사후 규정 준수를 넘어 시스템 설계 단계부터 개인정보보호를 내재화하는 원칙, Privacy by Design의 개요
...
**개념**: Ann Cavoukian 박사가 제안한 개인정보보호 접근 방식으로, 개인정보보호를 시스템·서비스·비즈니스 프로세스의 **설계 단계부터 기본값(Default)으로 내재화**하여 사후 조치가 아닌 사전 예방 중심의 프라이버시를 실현하는 프레임워크.

**특징**:
- GDPR Article 25 **"설계 및 기본 설정에 의한 데이터 보호"** 의 법적 기반이 된 개념.
- 기능성(Functionality)과 프라이버시를 상충 관계가 아닌 **양립 가능한 가치**로 접근.
- 기술적 조치(암호화·익명화)와 조직적 조치(정책·교육)를 통합한 전사적 프레임워크.
