...
## 1. 노드·엣지 모델링으로 구조적 관계 패턴을 분석하는 수학 프레임워크, 그래프 이론의 개요
...
**정의**: 개체(Entity)를 **노드(Node)**, 개체 간의 관계를 **엣지(Edge)** 로 표현하여 복잡한 연결 구조를 수학적으로 분석하는 이론 체계로, 지식 그래프·소셜 네트워크·추천 시스템·경로 최적화 등 다양한 데이터 분석 분야에 적용되는 기반 프레임워크.

**특징**:  
 **(다대다 관계 표현)** 관계형 데이터베이스로 표현이 어려운 **복잡한 다대다 관계**를 직관적으로 모델링.  
 **(다양한 그래프 유형)** 방향성(유향/무향), 가중치(유가중/무가중) 등 다양한 그래프 유형으로 실세계 문제 표현.  
 **(그래프 알고리즘)** BFS·DFS·Dijkstra 등 **그래프 알고리즘**을 통해 최단 경로·중심성·클러스터 분석 가능.  
