...
**정의**: 조직이 보유한 정형·비정형 데이터의 품질을 완전성, 정확성, 일관성, 유효성 등의 기준으로 측정·진단하고, 발견된 품질 문제를 체계적으로 개선·모니터링하는 데이터 관리 체계.

**특징**:  
 **(6대 품질 차원)** 데이터 품질을 완전성·정확성·일관성·유효성·적시성·유일성 6대 차원으로 정의하여 측정.  
 **(통합 품질 관리)** 정형 데이터(DB·DW)뿐 아니라 비정형 데이터(텍스트·이미지·로그)까지 포괄하는 통합 품질 관리.  
 **(사전 예방 전환)** 사후 정제(Cleansing) 중심에서 **사전 예방(Prevention)** 과 **지속 모니터링** 중심으로 전환.  
