---
sidebar_position: 2
title: ISMS-P
---

# ISMS-P
**Information Security & Personal Information Management System**

## 1. 한국형 정보보호 및 개인정보 보호 통합 인증, ISMS-P의 개요
```mermaid
flowchart LR
    A["이원화된 보안/개인정보 관리"] -- "통합 관리체계 인증 전환" --> B["ISMS-P 체계"]
```
**정의**: 정보보호 관리체계(ISMS)와 개인정보 보호 관리체계(PIMS)를 통합하여, 조직이 중요 정보자산을 안전하게 보호하고 개인정보 흐름을 체계적으로 관리하는지 인증하는 제도.

**특징**:  
 **(법정 의무)** 정보통신망법·개인정보보호법에 따라 일정 규모 이상 사업자에게 인증이 의무화.  
 **(통합 체계)** 정보보호(ISMS) + 개인정보보호(PIMS)를 하나의 심사로 통합하여 인증 효율성 제고.  
 **(102개 항목)** 관리체계 수립부터 개인정보 처리 단계별 보안까지 총 102개 인증 항목으로 구성.  

---

## 2. ISMS-P의 인증 기준 및 거버넌스 체계

### 가. 3개 영역 통합 관리 모델

```mermaid
flowchart TD
    subgraph AREA1["1. 관리체계 수립 및 운영 (16항목)"]
        A1["관리체계 설정 / 위험 관리 / 운영 및 개선"]
    end
    subgraph AREA2["2. 보호대책 요구사항 (64항목)"]
        A2["인적 보안 / 자산 관리 / 서비스 보안 / 사고 대응 등"]
    end
    subgraph AREA3["3. 개인정보 처리 단계별 요구사항 (22항목)"]
        A3["수집 / 이용 / 제공 / 파기 / 정보주체 권리보호"]
    end

    AREA1 --> AREA2
    AREA2 --> AREA3
    AREA3 --> AREA1
    
    style AREA1 fill:#E3F2FD,stroke:#1976D2
    style AREA3 fill:#FFF3E0,stroke:#F57C00
```

| 영역 | 주요 내용 | 인증 항목 수 |
|---|---|---|
| **관리체계 수립/운영** | 정책 수립, 조직 구성, 범위 설정, 위험 관리 | 16개 |
| **보호대책 요구사항** | 물리적/기술적 보안, 접근 통제, 암호화, 운영 보안 | 64개 |
| **개인정보 처리 단계** | 수집, 보유, 이용, 제공, 파기 시 보안 조치 | 22개 |

---

### 나. ISMS-P 인증 절차 및 생애주기

```mermaid
flowchart LR
    PRE["준비<br/>(범위 설정/GAP 분석)"]
    EST["구축/운영<br/>(정책 수립/보안 조치)"]
    AUDIT["심사<br/>(서면/현장 심사)"]
    CERT["인증<br/>(심의/인증서 교부)"]
    MAINT["유지<br/>(사후/갱신 심사)"]

    PRE --> EST --> AUDIT --> CERT --> MAINT
    MAINT --> AUDIT
```

| 단계 | 활동 내용 | 비고 |
|---|---|---|
| **위험 평가** | 자산 식별 및 위협/취약점 분석을 통한 위험 수용 수준 결정 | 핵심 단계 |
| **결함 조치** | 심사 시 발견된 부적합 사항에 대한 보완 조치 수행 | 보완 기간 준수 |
| **사후 심사** | 인증 유지 기간(3년) 중 매년 정기적 점검 수행 | 유지 관리 |

---

## 3. ISMS-P 도입의 기대효과 및 활용 방안

| 구분 | 주요 기대효과 | 활용 및 실무 적용 방안 |
|---|---|---|
| **법적 준거성** | 관련 법령(망법, 개법) 준수 입증 | 법정 의무 인증 대상 기업의 과태료 리스크 해소 |
| **보안 수준 제고** | 체계적인 정보보호 관리 프로세스 정착 | 상시적 위험 관리 체계 구축을 통한 보안 사고 예방 |
| **대외 신뢰도** | 기업 이미지 및 고객 신뢰 향상 | 공공기관 및 대기업 협력사 등록 시 보안 증빙으로 활용 |
| **보험료 할인** | 사이버 보험 등 부가 혜택 | 정보보호 공시 및 보험료 감면 혜택 연계 가능 |
