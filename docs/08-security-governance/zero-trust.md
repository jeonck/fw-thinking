...
```
**정의**: "절대 신뢰하지 말고, 언제나 검증하라(Never Trust, Always Verify)"는 원칙 아래, 네트워크 내외부를 불문하고 모든 접근 요청에 대해 최소 권한과 지속적 인증을 적용하는 보안 모델.

**특징**:  
 **(경계 해체)** 내부 네트워크도 신뢰하지 않는 Never Trust Always Verify 원칙으로 경계 보안의 한계를 극복.  
 **(최소 권한)** 최소 권한 원칙(PoLP)으로 필요한 자원에만 최소한의 권한을 부여하여 침해 범위를 제한.  
 **(컨텍스트 인증)** 사용자·기기·위치·행동 컨텍스트를 동적으로 분석하여 매 요청마다 신뢰를 재검증.  
