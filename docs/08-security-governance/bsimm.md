...
## 1. 실제 기업의 SW 보안 활동을 관찰·측정하여 업계 평균과 비교·벤치마킹하는 데이터 기반 성숙도 모델, BSIMM의 개요
...
**개념**: Synopsys(구 Cigital)가 금융·핀테크·헬스케어·기술 기업 등 수백 개 조직의 **실제 SW 보안 활동을 관찰·측정** 하여 구축한 데이터 기반 성숙도 모델로, 조직의 SW 보안 프로그램을 **4대 영역·12개 실천·122개 활동** 으로 분류하고 업계 평균과 비교하여 현재 수준과 개선 방향을 제시하는 서술적(Descriptive) 프레임워크.

**특징**:
- **관찰 기반(Observational)**: 이상적 목표가 아닌 업계에서 실제 수행 중인 활동을 측정·비교.
- OWASP SAMM이 **규범적(Prescriptive)** 로드맵을 제시한다면 BSIMM은 **서술적(Descriptive) 벤치마킹** 에 특화.
- 매년 또는 격년으로 참여 기업 데이터 업데이트 — 최신 업계 트렌드가 자동 반영.
