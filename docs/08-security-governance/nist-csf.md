...
```mermaid
flowchart LR
    A["규제 준수 중심 보안"] -- "결과 및 리스크 기반 보안 전환" --> B["NIST CSF 체계"]
```
**정의**: 미국 국립표준기술연구소(NIST)에서 제정한 프레임워크로, 조직이 사이버 보안 리스크를 이해, 관리 및 표현할 수 있도록 돕는 공통 언어와 체계.

**특징**:  
 **(유연한 구조)** 특정 산업이나 규모에 관계없이 자율적으로 적용 수준을 선택하는 비즈니스 중심 구조.  
 **(3대 구성)** Core(기능)·Tiers(성숙도)·Profiles(현재/목표)의 3요소로 사이버 보안 역량을 체계화.  
 **(거버넌스 강화)** CSF 2.0에서 Govern 기능을 신설하여 최고경영진 책임과 거버넌스를 명시적으로 강화.  
