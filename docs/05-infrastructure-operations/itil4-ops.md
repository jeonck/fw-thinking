...
```mermaid
flowchart LR
    A["장애 대응형 운영"] -- "가치 공동 창출 및 예방형 운영 전환" --> B["ITIL 4 Operations"]
```
**정의**: 고객에게 합의된 수준의 가치를 지속적으로 제공하기 위해 IT 서비스의 일상적인 운영과 관리를 담당하는 ITIL 4의 핵심 실무 영역.

**특징**:  
 **(프랙티스 중심)** ITIL v3 프로세스에서 유연한 프랙티스(Practices) 중심으로 전환하여 적용 범위를 확장.  
 **(예방적 운영)** 장애 대응(Reactive)을 넘어 모니터링·분석 기반의 예방적(Proactive) 운영을 실현.  
 **(가치 공동창출)** 서비스 제공자와 소비자가 함께 가치를 창출하는 공동 창출 모델을 강조.  
