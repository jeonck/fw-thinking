...
## 1. IT 자산 생애주기를 추적·관리하여 비용과 리스크를 최적화하는 체계, ITAM의 개요
```mermaid
flowchart LR
    A["개별 자산 운영"] -- "생애주기 관리 기반 전환" --> B["IT 자산 최적화"]
```
**정의**: IT 자산의 조달부터 폐기까지의 전 생애주기를 추적, 관리하여 비용을 절감하고 가용성을 최적화하는 관리 체계.

**특징**:  
 **(자산 식별·추적)** 자산의 식별, 추적, 상태 관리를 통한 투명성 확보.  
 **(라이선스·보안)** 라이선스 준수 및 보안 리스크 예방.  
