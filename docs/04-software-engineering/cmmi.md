...
```mermaid
flowchart LR
    A["미관리된 개별 프로세스"] -- "정량적 관리 및 최적화 전환" --> B["CMMI 성숙도 체계"]
```
**정의**: 조직의 소프트웨어 개발 및 서비스 프로세스 능력을 평가하고 개선하기 위한 단계별 가이드라인이자 통합 프로세스 성숙도 모델.

**특징**:  
 **(품질·비용)** 체계적 프로세스 개선으로 SW 품질을 높이고 개발 비용·일정 편차를 줄임.  
 **(성숙도 정량화)** 1~5단계 성숙도 수준으로 조직 프로세스 역량을 객관적으로 측정·비교.  
 **(지속 개선)** 조직 전체의 프로세스를 표준화·최적화하는 지속적 개선 문화를 정착시킴.  
