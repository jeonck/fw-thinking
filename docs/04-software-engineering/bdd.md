...
## 1. 비즈니스 행위를 자연어 시나리오로 명세하여 개발자·QA·비즈니스 간 공통 이해를 기반으로 소프트웨어를 개발하는 방법론, BDD의 개요
...
**개념**: TDD를 비즈니스 관점으로 확장한 개발 방법론으로, 시스템이 수행해야 할 **행위(Behavior)** 를 비즈니스 이해관계자가 읽을 수 있는 **Given-When-Then 자연어 시나리오**로 명세하고, 이를 자동화 테스트로 연결함으로써 개발자·QA·비즈니스 전문가 간의 공통 이해를 형성하는 협업 중심 개발 방법론.
