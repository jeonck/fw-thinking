...
**특징**:  
 **(설계 명세 역할)** 테스트가 설계 명세(Specification) 역할을 겸하여 요구사항과 구현의 정합성 보장.  
 **(조기 결함 발견)** 짧은 피드백 루프(수 분 이내)로 결함을 **조기에 발견·수정** 하여 기술 부채 억제.  
 **(리팩토링 보호)** 리팩토링 안전망으로서의 테스트 코드 — 구조 개선 시 기존 동작 보장.  
