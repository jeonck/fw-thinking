...
```mermaid
flowchart LR
    A["경직된 계획 중심 개발"] -- "반복적 피드백 및 가치 중심 전환" --> B["Agile & Scrum"]
```
**정의**:
...
**특징**:  
 **(빠른 피드백)** 짧은 스프린트(2~4주) 주기로 고객 피드백을 빠르게 반영하여 방향을 조정.  
 **(지속 전달)** 매 스프린트마다 작동 가능한 소프트웨어를 전달하여 비즈니스 가치를 지속 창출.  
 **(품질 내재화)** 스프린트 리뷰·회고를 통해 프로세스와 품질을 반복적으로 개선.  
