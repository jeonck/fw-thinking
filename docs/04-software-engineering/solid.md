...
## 1. 변경에 강한 SW를 위한 객체지향 설계의 5대 핵심 원칙, SOLID의 개요
...
**정의**: Robert C. Martin이 정립한 객체지향 설계의 5가지 핵심 원칙으로, **단일 책임(SRP), 개방-폐쇄(OCP), 리스코프 치환(LSP), 인터페이스 분리(ISP), 의존성 역전(DIP)** 을 통해 결합도를 낮추고 응집도를 높여 유지보수·확장이 용이한 소프트웨어를 설계하는 지침.

**특징**:  
 **(상호 보완 원칙)** 각 원칙은 독립적이지만 상호 보완적으로 작용하여 **클린 아키텍처·DDD·디자인 패턴**의 기반 원칙.  
 **(코드 스멜 지표)** 원칙 위반은 코드 스멜(Code Smell)로 나타나며 기술 부채를 유발.  
 **(비용-편익 판단)** 과도한 적용은 복잡성을 높일 수 있으므로 **비용-편익 판단** 필요.  
