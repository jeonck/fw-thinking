...
```
**정의**: 기업의 정보와 기술(I&T) 거버넌스 체계 구축을 위해 비즈니스 목적과 IT 관리를 정렬하는 EGIT(Enterprise Governance of IT) 프레임워크.

**특징**:  
 **(맞춤형 설계)** Design Factors(설계 요인) 도입으로 조직 특성에 맞는 거버넌스 Tailoring을 지원.  
 **(성능 관리)** CMMI 기반 성능 관리 모델을 적용하여 프로세스 성숙도를 정량적으로 측정.  
 **(유연한 확장)** 40개 거버넌스 목표를 모듈 방식으로 선택·조합하여 규모에 무관하게 적용 가능.  
