...
**특징**:  
 **(내부 자원 접근)** 외부 환경 분석(Porter's 5 Forces) 중심의 산업 구조 분석을 보완하는 내부 자원 중심의 접근법.  
 **(단계적 우위 분류)** 네 가지 기준의 충족 여부에 따라 경쟁 열위 → 경쟁 균형 → 일시적 우위 → 지속적 우위로 단계적 분류.  
 **(무형 자원 평가)** IT 시스템, 데이터, 인적 역량 등 무형 자원의 전략적 가치 평가에 효과적으로 적용 가능.  
