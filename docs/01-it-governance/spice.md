...
## 1. SPICE: SW 프로세스 역량 평가 및 체계적 개선

```mermaid
flowchart LR
    A["전통적 SW 개발"] -- "프로세스 수행 능력 개선" --> B["SPICE (ISO/IEC 15504)"]
```
**핵심**: 소프트웨어 프로세스 개선과 능력 결정을 위한 국제 표준으로, 조직의 프로세스 수행 능력을 체계적으로 평가하고 개선하는 프레임워크.

**특징**:  
 **(수행 능력 차원)** 프로세스 성숙도를 6단계로 정의하여 평가.  
 **(프로세스 차원)** 조직의 프로세스 범위를 분류하여 관리.  
