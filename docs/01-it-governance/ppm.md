...
## 1. PPM: 전략 목표 달성을 위한 프로젝트 포트폴리오 관리

```mermaid
flowchart LR
    A["개별 프로젝트 수행"] -- "전략 목표 기반 프로젝트 포트폴리오 통합" --> B["PPM (Project Portfolio Management)"]
```
**핵심**: 기업의 전체 프로젝트를 하나의 포트폴리오로 관리하여, 비즈니스 가치를 극대화하고 리스크를 최소화하는 관리 체계.

**특징**:  
 **(우선순위·자원 배분)** 프로젝트 우선순위 선정 및 자원 배분 최적화.  
 **(전사 투자 효율)** 개별 프로젝트를 넘어 전사적 관점의 IT 투자 효율 관리.  
