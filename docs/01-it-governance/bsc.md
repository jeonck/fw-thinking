...
```mermaid
flowchart LR
    A["재무 중심 성과 측정"] -- "다각적 균형 성과 관리 전환" --> B["BSC 프레임워크"]
```
**정의**: 재무적 지표 중심의 성과 측정에서 벗어나, 고객, 내부 프로세스, 학습과 성장 등 비재무적 관점을 포함하여 조직의 전략을 균형 있게 관리하는 전략 경영 시스템.

**특징**:  
 **(인과관계 가시화)** 전략 맵(Strategy Map)으로 목표 간 인과관계를 시각화하여 전략 논리를 명확화.  
 **(KPI 연계)** 4대 관점별 핵심 성과 지표(KPI)를 도출하여 정량적 성과 측정 기반 마련.  
 **(전략 실행력)** 비전과 전략을 현장 실행 단위로 분해하여 조직 전체의 전략 정렬을 강화.  
