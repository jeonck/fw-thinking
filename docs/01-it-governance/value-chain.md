...
## 1. 포터의 가치 사슬: 개별 활동 분석에서 통합적 가치 창출 분석으로

```mermaid
flowchart LR
    A["개별 활동별 분석"] -- "가치 창출 관점의 활동 통합 분석" --> B["포터의 가치 사슬"]
```
**핵심**: 기업 활동을 가치 창출 단계별로 통합 분석하여 경쟁 우위의 원천을 파악하고 최적화하는 프레임워크.

**특징**:  
 **(본원·지원 활동)** 기업 활동을 본원적 활동(5개)과 지원 활동(4개)으로 구분하여 부가가치 창출 원천을 분석.  
 **(원가·차별화 전략)** 원가 우위 및 차별화 전략 수립의 기반 제공.  
