...
**특징**:  
 **(탈중앙화)** 중앙 기관 없이 P2P 네트워크가 분산 합의로 데이터를 관리하여 단일 실패점을 제거.  
 **(불변성)** 한번 기록된 데이터는 변경·삭제가 불가능하여 투명성과 데이터 무결성을 보장.  
 **(스마트 계약)** 조건 충족 시 자동 실행되는 Smart Contract로 중개자 없는 비즈니스 자동화를 실현.  
