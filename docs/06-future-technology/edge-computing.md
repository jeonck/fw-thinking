...
```
**정의**: 데이터를 중앙 클라우드 서버로 보내지 않고, 데이터가 발생한 물리적 장소(에지) 또는 그 근처에서 실시간으로 처리하는 분산 컴퓨팅 기술.

**특징**:  
 **(저지연 처리)** 데이터를 발생 현장 근처에서 처리하여 클라우드 왕복 없이 실시간 응답을 실현.  
 **(대역폭 절감)** 로컬 처리로 클라우드 전송 데이터를 줄여 통신 비용과 네트워크 부하를 절감.  
 **(MEC 진화)** 5G/6G와 결합한 MEC(Multi-access Edge Computing)로 자율주행·스마트 팩토리 핵심 인프라로 진화.  
