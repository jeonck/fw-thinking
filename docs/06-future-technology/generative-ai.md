...
**특징**:  
 **(LLM 중심)** 대규모 언어 모델(LLM)을 기반으로 텍스트·코드·이미지 등 다양한 콘텐츠를 생성.  
 **(프롬프트 엔지니어링)** 입력 방식에 따라 출력 품질이 크게 달라져 프롬프트 엔지니어링이 핵심 역량으로 부상.  
 **(멀티모달)** 텍스트·이미지·음성·영상을 통합 처리하는 멀티모달 역량으로 적용 영역을 급속히 확장.  
