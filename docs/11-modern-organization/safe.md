...
```mermaid
flowchart LR
    A["개별 애자일 팀"] -- "전사적 확장 및 정렬 전환" --> B["SAFe (Scaled Agile Framework)"]
```
**정의**: 대규모 엔터프라이즈 환경에서 수십, 수백 명의 인원이 협업하여 복잡한 시스템을 개발하기 위해 애자일과 린(Lean) 원칙을 확장한 프레임워크.

**특징**:  
 **(ART 중심)** Agile Release Train(ART)으로 여러 팀이 공동 케이던스로 동기화된 가치를 전달.  
 **(전략 정렬)** 포트폴리오→솔루션→프로그램→팀 4개 계층으로 전략과 실행을 수직 정렬.  
 **(정교한 구조)** 역할·이벤트·산출물을 명확히 정의하여 대규모 조직에서도 애자일을 체계적으로 운영.  
