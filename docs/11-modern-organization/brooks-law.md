...
**특징**:  
 **(커뮤니케이션 오버헤드)** 소프트웨어 개발은 순수 병렬화가 어렵고, 인력 간 커뮤니케이션 오버헤드가 인원 수의 제곱에 비례하여 증가.  
 **(온보딩 비용)** 신규 투입 인력의 **온보딩·교육** 에 기존 팀원의 시간과 에너지가 소모.  
 **(Man-Month 한계)** 공수(Man-Month)는 교환 가능한 단위가 아님 — "9명이 1개월에 할 일을 1명이 9개월에 할 수 없는 것처럼, 그 역도 성립하지 않는다".  
