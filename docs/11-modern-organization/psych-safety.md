...
**특징**:  
 **(고기준 안전 환경)** 심리적 안전감은 안락함(Comfort)이 아님 — 낮은 기준·낮은 책임이 아닌 높은 기준 속 안전한 환경이 목표.  
 **(리더십 영향)** 리더십 행동이 팀의 심리적 안전감 수준에 가장 큰 영향을 미침.  
 **(Project Aristotle)** Google Project Aristotle: 심리적 안전감이 팀 효과성을 결정하는 **5대 요인 중 1위** (다음은 의존성·구조·의미·영향).  
