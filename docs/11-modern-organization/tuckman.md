...
**특징**:  
 **(단계 순서 불변)** 모든 팀은 반드시 이 순서대로 단계를 거침 — 건너뛸 수 없으며 퇴행도 가능(멤버 교체·목표 변경 시 Storming 재발).  
 **(리더십 스타일)** 리더는 각 단계에서 요구되는 **리더십 스타일이 다름** — 초기는 지시적, 후반은 위임적.  
 **(인식 효과)** 단계를 인식하는 것만으로도 팀원·리더 모두의 **불안과 갈등 수용력** 이 높아짐.  
