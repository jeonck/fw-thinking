...
**특징**:  
 **(ADM 순환)** Architecture Development Method(ADM)라는 순환형 프로세스로 EA를 단계적으로 개발·관리.  
 **(콘텐츠 프레임워크)** 아키텍처 산출물·아티팩트를 체계적으로 분류하는 콘텐츠 프레임워크 제공.  
 **(참조 모델)** TRM·III-RM 등 참조 모델로 아키텍처 표준화와 재사용성을 높임.  
