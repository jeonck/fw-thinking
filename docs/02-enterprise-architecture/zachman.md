...
```mermaid
flowchart LR
    A["파편화된 시스템 설계"] -- "전사적 관점의 분류 체계 확립" --> B["Zachman Framework"]
```
**정의**: 기업의 복잡한 정보 인프라를 다각도에서 바라보고 분류하기 위해 6개의 관점(Rows)과 6개의 핵심 질문(Columns)으로 구성된 6x6 매트릭스 형태의 분류 체계(Taxonomy).

**특징**:  
 **(분류 체계)** 방법론이 아닌 6×6 분류 체계(Schema)로서 아키텍처 산출물을 빠짐없이 구조화.  
 **(셀 유일성)** 각 셀은 유일하고 상호 배타적인 속성을 가져 중복 없는 완전한 분류를 보장.  
 **(다관점 표현)** What·How·Where·Who·When·Why 6개 질문과 6개 이해관계자 관점을 교차하여 입체적 아키텍처 표현.  
