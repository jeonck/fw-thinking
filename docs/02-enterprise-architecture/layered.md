...
## 1. 관심사별 수평 계층 분리로 변경 영향을 계층 내로 제한하는 아키텍처, 계층형 아키텍처의 개요
...
**정의**: 소프트웨어 시스템을 **관심사(Concern)** 에 따라 수평적인 계층(Layer)으로 분리하고, 각 계층이 바로 하위 계층에만 의존하도록 설계하는 가장 보편적인 아키텍처 패턴으로, 변경 영향을 계층 내로 제한하고 각 계층을 독립적으로 개발·테스트·교체 가능하게 하는 구조.

**특징**:  
 **(단방향 의존성)** 상위 계층은 하위 계층을 사용하지만 하위 계층은 상위 계층을 알지 못함.  
 **(관심사 분리(SoC))** 각 계층은 하나의 역할에만 집중하여 응집도 향상·결합도 감소.  
 **(주요 프레임워크 기반)** 엔터프라이즈 Java(Spring MVC), .NET, Django 등 주요 프레임워크의 **기본 구조적 패턴**.  
