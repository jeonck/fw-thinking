...
## 1. 테스트 프로세스 성숙도를 5단계로 진단·개선하는 품질 프레임워크, TMMi의 개요
...
**정의**: TMMi Foundation이 CMMI를 기반으로 개발한 테스트 전문 성숙도 모델로, 소프트웨어 테스트 프로세스를 **5단계 성숙도 수준** 과 각 수준별 **프로세스 영역(PA)** 으로 구조화하여 조직의 현행 테스트 역량을 진단하고 단계적 개선 로드맵을 제시하는 프레임워크.

**특징**:  
 **(단계적 성숙도)** CMMI의 단계적(Staged) 표현 방식 채택 — Level 1~5의 순차적 성숙도 향상.  
 **(전 영역 포괄)** 테스트 정책·전략·계획·설계·실행·측정·최적화 전 영역을 구조적으로 포괄.  
 **(표준 연계)** ISO 29119(소프트웨어 테스트 표준)·ISTQB와 상호 보완적으로 활용 가능.  
