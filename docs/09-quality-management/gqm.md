...
## 1. 측정 목표를 먼저 정의하고 질문과 지표를 역방향으로 도출하는 체계적 SW 품질 측정 프레임워크, GQM의 개요
...
**개념**: Victor Basili가 개발한 소프트웨어 측정 프레임워크로, **목표(Goal)→질문(Question)→지표(Metric)** 의 3계층 하향식 접근으로 측정 목적과 직결된 의미 있는 품질 지표를 도출하고, 수집된 데이터를 역방향으로 해석하여 목표 달성 여부를 평가하는 체계적 측정 전략.

**특징**:
- **Top-Down 설계**: 지표를 먼저 정하는 것이 아닌 측정 **목표를 먼저 정의** 하고 필요한 지표를 역방향으로 도출.
- **맥락 의존성**: 동일한 지표라도 측정 목표·관점·환경에 따라 해석이 달라짐 — 맥락을 항상 명시.
- ISO/IEC 15939(소프트웨어 측정 표준)·CMMI·Six Sigma DMAIC의 Measure 단계와 연계.
