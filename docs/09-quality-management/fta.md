...
**특징**:  
 **(연역적 분석)** 최상위 고장(Top Event)에서 출발하여 원인을 하향 추적하는 연역적 접근으로 체계적 분석.  
 **(AND·OR 논리)** 논리 게이트(AND/OR)로 원인 조합을 모델링하여 고장 발생 확률을 정량적으로 산출.  
 **(위험 완화)** 복잡한 시스템의 잠재 위험 요소를 사전에 식별하여 선제적 완화 대책 수립에 활용.  
