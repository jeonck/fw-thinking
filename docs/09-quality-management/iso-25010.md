...
```mermaid
flowchart LR
    A["주관적 품질 평가"] -- "정량적/체계적 품질 모델 전환" --> B["ISO/IEC 25010"]
```
**정의**: 소프트웨어 제품의 품질을 평가하기 위해 품질 특성(Characteristics)과 부특성(Sub-characteristics)을 정의한 국제 표준(ISO/IEC 9126의 후속).

**특징**:  
 **(이원 모델)** 제품 품질(8특성)과 사용 품질(5특성)의 이원 모델로 SW 품질을 다각도로 평가.  
 **(정량 측정)** 각 품질 특성에 대한 객관적·정량적 측정 지표를 제공하여 품질 비교·추적을 지원.  
 **(국제 표준)** ISO 9126 후속 표준으로 글로벌 SW 품질 평가의 공통 언어를 제공.  
