...
## 1. 잠재적 고장을 사전에 식별하고 위험도 기반으로 선제 대응하는 신뢰성 분석 기법, FMEA의 개요
...
**개념**: 제품·프로세스·시스템에서 발생할 수 있는 잠재적 고장 모드(Failure Mode)를 체계적으로 식별하고, 각 고장의 영향(Effect)·원인(Cause)을 분석하여 **심각도(S)×발생도(O)×검출도(D)** 로 위험 우선순위 번호(RPN)를 산출함으로써 사전 예방 대책을 수립하는 품질·신뢰성 분석 기법.
